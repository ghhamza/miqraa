...
class DFTextComponent extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.quranService.promise.then((respone) => {
      ...
      scheduleDraw();
      this._mutationObserver = new MutationObserver(() => scheduleDraw());
  ...
  drawText() {
    const text = this.getLineText();
    if (!this.quranShaper || !text) return;
    ...
    if (autoWidth) {
      const qswidth = this.quranShaper.shapeText(text, 0, scale, false, false, false, this.ctx);
      width = qswidth * scale;
    } else {
    ...
    this.quranShaper.shapeText(text, wd, scale, this.just, this.tajweed, this.fontExpansion, this.ctx);
